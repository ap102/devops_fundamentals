...
docker run --name ap-jenkins -p 8080:8080 --network postgres-sonarqube -d jenkins/jenkins
#create and connect sonatype/nexus container
docker run --name ap-sonarqube -p 8081:8081 --network postgres-sonarqube -d sonatype/nexus3
